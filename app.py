...
# ---- 2. API KEY HILFSFUNKTION (PRODUKTIV FÜR STREAMLIT CLOUD) ----
def lade_api_key():
    # Versucht den Key direkt aus den Streamlit Secrets zu laden
    if "GEMINI_API_KEY" in st.secrets:
        return st.secrets["GEMINI_API_KEY"]
    return ""

# ---- 3. KI FUNKTIONEN MIT GLOBALEM INTELLIGENTEM RATE-LIMITER ----
...
ki_model = initialisiere_ki(gespeicherter_key) if gespeicherter_key else None
...
if ist_admin:
    ...
    # Kicken-Funktion im Admin-Bereich
    ...
    if rangliste.empty:
        ...
        st.write("---")
        # Rangliste mit formatierten Zeiten anzeigen
    ...
    if "schueler_name" not in st.session_state:
        ...
        # Überprüfen, ob das Team heimlich gekickt wurde
        ...
        if satz_idx >= len(SAETZE):
            ...
            if satz_idx == 1: 
                ...
                *Der Clou:* Da die Römer rückwärts gezählt haben, bedeutet *'Nonum kal. Septembres'* wörtlich 'der neunte Tag vor dem 1. September' (wobei sie den Start- und Endtag mitzählen). Wenn du im Kalender 9 Tage vom 1. September zurückgehst, landest du genau beim geschichtsträchtigen **24. August**!
                """)
            ...
            with col1:
                ...
                if hilfe_aktiviert:
                    ...
                    # Teilt die Vokabeln auf und gibt sie ordentlich untereinander aus
                        ...
                        if st.button("Fragen (-50 Pkt)", key=f"kibtn_{satz_idx}"):
                            if ki_frage.strip() == "":
                                ...
                                with st.spinner("KI analysiert..."):
                                    antwort = frage_ki_assistent(ki_model, ki_frage, aktuelle_daten)
            ...
            if st.button("Satz automatisch von KI überprüfen lassen 🤖✅", key=f"check_btn_{satz_idx}"):
                if not ki_model:
                    ...
                    with st.spinner("Eure Übersetzung wird bewertet..."):
                        punkte, feedback = bewerte_uebersetzung(ki_model, uebersetzung, aktuelle_daten)
