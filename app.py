...
# --- 2. DIE INHALTLICHEN DATENSÄTZE (AUSFÜHRLICH) ---
SAETZE = [
    {
        "satz_id": 1, 
...
if page == "Admin-Panel":
    st.sidebar.title("Admin-Bereich")
    ...
    if not st.session_state.user_name:
        ...
        if idx < len(SAETZE):
            ...
            eingabe = st.text_area("Deine Übersetzung:", key=f"in_{idx}")
            ...
            # KI Assistenz Eingabe
            frage = st.text_input("Frage an den KI-Lehrer:")
            if st.button("KI fragen"):
                st.write(model.generate_content(f"Erkläre kurz: {frage}").text)
